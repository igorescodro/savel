...
internal class MusicBrainzApi(
    ...
    private suspend fun getArtistWithResourceIds(artistList: List<RepoArtist>): List<Artist> = coroutineScope {
        val deferredResponses = artistList.map { artist ->
            async {
                val response = httpClient.client.get("$URL${artist.id}") {
                    parameter("fmt", "json")
